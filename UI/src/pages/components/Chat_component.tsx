...
export default function Chat_component() {
    ...
    const deleteMessages = async (id: number) => {
        console.log("JWT Token:", jwt);
        try {
            const response = await axios.post("http://localhost:8080/api/message/delete/", {
                id: id
            }, {
                headers: {Authorization: `Bearer ${jwt}`},
            });
              ...
              <>
                {message?.sender !== "me" ? (
                  ...
                  <div className="flex justify-end mb-4 cursor-pointer">
                    ...
                    <div className="h-[80vh] overflow-y-auto p-4 pb-36">
                        {allMessages.map((message: allMessages) => {
                            return (<>
                                    {message.fromMessage !== "me" ? (<div className="flex mb-4 cursor-pointer">
                                            <button onClick={() => {
                                        ...
                                        </div>) : (<div className="flex justify-end mb-4 cursor-pointer">
                                            <button onClick={() => {
                                                deleteMessages(message.id)
                                            }}>*
                                            </button>
                                            <div
                                                className="flex max-w-96 bg-indigo-500 text-white rounded-lg p-3 gap-3">
                                                <p>{message.toMessage}</p>
                                            </div>
                                            <div className="w-9 h-9 rounded-full flex items-center justify-center ml-2">
                                                <img
                                                    src="https://placehold.co/200x/b7a8ff/ffffff.svg?text=ʕ•́ᴥ•̀ʔ&font=Lato"
